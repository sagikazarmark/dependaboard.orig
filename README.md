...
12. Once a batch has finished, open **Batches** and confirm it is listed with its counts, requester, and a working link per target; confirm it is still listed after `BulkAction/progress` for its id has stopped returning state. Then paste its `?batch=<id>` link into a fresh tab and confirm the drawer opens on the record at once, with every target's verdict, without waiting on Restate; paste a link with a well-formed id no batch has and confirm the drawer says the projection has no such batch and is asking Restate, and that after thirty seconds a toast says no batch by that id was found anywhere.
