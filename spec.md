...
read is a configuration `Fatal`. "Read" includes the object's own snapshot: a mutation's
step is always told the resource is known, because a mutation cannot reach GitHub without
passing the guard that proves the object read it, while `sync` tells its step what it
actually found. So a pull request that has gone since the table showed it is
`Rejected(NotFound)` — skipped by retry, explained by the drawer — not a failure that asks
for the operator. The same read-first logic governs a refused write: a
