...
These are what the existing issues (see #50–#54) already do; keep to them.

- **One issue, one commit.** A ticket is scoped to land as a single commit whose subject reads as the behaviour it delivers, in the same voice as the issue title.
- **Body shape**, in this order of headings:
  - `## What to build` — the problem as the user meets it, then the change in one paragraph.
  - `## Decision needed` — the open design questions, each as a bullet with its options. Omit the heading when there are none.
  - `## Acceptance criteria` — a checklist. When there is a *Decision needed* section, the first item is `Decisions recorded in the issue`. Include a test line and, for user-visible changes, a line adding the case to the README's *Live Acceptance* checklist.
